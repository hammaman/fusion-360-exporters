...
class TotalExport(object):
  def __init__(self, app):
    ...
    # New features
    # Start from project number specified below
    self.project_start = 0
    # Ignore sketches for the components in this list:
    # This may be required if the export freezes on exporting the sketches for particular components
    self.components_to_ignore_dxf = []
  ...
  def _export_data(self, output_path):
    ...
    for hub_index in range(all_hubs.count):
      ...
      for project_index in range(all_projects.count):
        ...
        # New code
        if project_index < self.project_start:
          continue
  ...
  def _write_component(self, component_base_path, component: adsk.fusion.Component):
    ...
    # New code to ignore specified components
    if component.name in self.components_to_ignore_dxf:
      self.log.info("Ignoring dxf sketch files for \"{}\"".format(component.name))
    else:
  ...
  def _write_iges(self, output_path, component: adsk.fusion.Component):
    ...
    try:
      ...
      self.log.info('Iges file not created. Error of {}'.format(ex))
